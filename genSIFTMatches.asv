function [xs, xd] = genSIFTMatches(imgs, imgd, impl)
    if nargin < 3 || isempty(impl)
        impl = 'VLFeat';
    end

    if strcmpi(impl, 'VLFeat')
...
%         end
        ...
        [fd, vptd] = extractFeatures(imgd, pd);
        index_pairs = matchFeatures(fs, fd, MatchThreshold=20, MaxRatio=0.7);
        xs = vpts.Location(index_pairs(:,1),:);
        xd = vptd.Location(index_pairs(:,2),:);
    else
        fprintf(2, 'Invalid implementation %s chosen in genSIFTMatches.\n', impl);
        fprintf(2, "Should be either 'MATLAB' or 'VLFeat'.\n");
    end
end
